...
from utils.mcp_utils import execute_tool
from utils.parsers import ToolXMLParser
from utils.rubrics import HUDBaseRubric
...
def load_environment(
    ...
    **kwargs,
    ...
    Args:
        taskset: HuggingFace dataset identifier or path to JSON file (required)
        config_path: Path to config file (required)
        num_tasks: Optional limit on number of tasks to load
        split: Dataset split to load (default: train, ignored for JSON files)
        **kwargs: Additional arguments passed to HUDGym

    Returns:
        HUDGym: Configured environment
    """
    # Check if taskset is a JSON file path
    if taskset.endswith('.json'):
        # Load from JSON file
        with open(taskset) as f:
            data = json.load(f)

        # Detect format and convert to HuggingFace Dataset
        if isinstance(data, list):
            # Format 2: List of dictionaries (each dict is a complete task)
            hf_dataset = Dataset.from_list(data)
        elif isinstance(data, dict):
            # Format 1: HuggingFace format (dict with parallel arrays)
