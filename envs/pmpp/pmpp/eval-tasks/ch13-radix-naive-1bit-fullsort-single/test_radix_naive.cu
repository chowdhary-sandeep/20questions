...
static void fill_pattern(std::vector<unsigned int>& v, int pat) {
    ...
    switch (pat % 6) {
        ...
        case 3: // Many duplicates
            for (size_t i = 0; i < v.size(); ++i) v[i] = (unsigned int)(i % 7);
            break;
        case 4: // Single value
            for (size_t i = 0; i < v.size(); ++i) v[i] = 42;
            break;
        case 5: // Powers of 2
            for (size_t i = 0; i < v.size(); ++i) v[i] = 1u << (i % 32);
            break;
    }
}
...
// --- Test ---
int main() {
    printf("radix-naive-1bit-fullsort-single tests\n");
