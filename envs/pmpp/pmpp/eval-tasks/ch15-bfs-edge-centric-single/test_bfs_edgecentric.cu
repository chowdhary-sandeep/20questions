// ch15-bfs-edge-centric-single / test_bfs_edgecentric.cu
...
int main(){
    printf("ch15-bfs-edge-centric-single tests\n");
    std::vector<CSR> gs;
    gs.push_back(make_chain(32));        // linear topology, few edges per level
    gs.push_back(make_star(128));        // all edges activate simultaneously after level 1
    gs.push_back(make_grid2d(8,8));      // structured, moderate edge parallelism
    gs.push_back(make_erdos(200,0.05f)); // random, varying edge workload per level
    ...
    for(size_t gi=0; gi<gs.size(); ++gi){
        ...
        bfs_edge_centric_gpu(d_row, d_col, V, E, src, d_lvl);
