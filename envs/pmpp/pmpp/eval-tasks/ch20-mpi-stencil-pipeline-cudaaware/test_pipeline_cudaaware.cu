    ...
    for(int k=0;k<dimz;++k){
        for(int j=0;j<dimy;++j){
            for(int i=0;i<dimx;++i){
                if(i<4 || i>=dimx-4 || j<4 || j>=dimy-4 || k<4 || k>=dimz-4){
...
int main(){
    printf("ch20-mpi-stencil-pipeline-cudaaware tests\n");
    ...
    for(const auto& cs : cases){
        ...
        mpi_stencil_pipeline_cudaaware(d_in, d_out, dimx,dimy,dimz, procs);
