...
extern "C" void radix_sort_coarsened_host(unsigned int* data, int n);
...
static void fill_pattern(std::vector<unsigned int>& v, int pat) {
    ...
    switch (pat % 6) {
        ...
        case 2: // Descending
            ...
            break;
        case 3: // Many duplicates (good for coarsening test)
            for (size_t i = 0; i < v.size(); ++i) v[i] = (unsigned int)(i % 31);  // 31 buckets
            break;
        case 4: // Single value
            for (size_t i = 0; i < v.size(); ++i) v[i] = 42;
            break;
        case 5: // Bit patterns that test coarsening
            for (size_t i = 0; i < v.size(); ++i) {
                v[i] = ((unsigned int)i << 16) | ((unsigned int)(i % 16) << 8) | (unsigned int)(i % 256);
            }
            break;
    }
}
...
// --- Test ---
int main() {
    printf("radix-multiradix-coarsened-fullsort-single tests\n");
    ...
    const Case cases[] = {
        ...
        {10000, "n=10000"},
        {65536, "n=65536"},
        {100000, "n=100000"}  // Larger test for coarsening benefits
    };
    ...
    for (const auto& cs : cases) {
        for (int pat = 0; pat < 6; ++pat) {
            ...
            // Sort
            radix_sort_coarsened_host(d_data, n);
