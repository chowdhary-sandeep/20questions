// BAD: Corrupts input matrices (violates const-correctness)
                                     ...
                                     int M, int N, int K)
{
    ...
    for (int t = 0; t < tiles; ++t) {
        ...
        // BAD: Corrupt input matrices
        if (aRow < M && aCol < N && threadIdx.x == 0 && threadIdx.y == 0) {
            const_cast<float*>(A)[aRow * N + aCol] = 999.0f;
        }
        if (bRow < N && bCol < K && threadIdx.x == 1 && threadIdx.y == 0) {
            const_cast<float*>(B)[bRow * K + bCol] = 888.0f;
        }
