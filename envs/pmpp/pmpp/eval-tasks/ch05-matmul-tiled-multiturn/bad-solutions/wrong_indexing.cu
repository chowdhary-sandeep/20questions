// BAD: Wrong indexing in matrix access
                                     ...
                                     int M, int N, int K)
{
    ...
    for (int t = 0; t < tiles; ++t) {
        ...
        // BAD: Wrong matrix indexing (swapped dimensions)
        As[threadIdx.y][threadIdx.x] =
            (aRow < M && aCol < N) ? A[aCol * M + aRow] : 0.0f;  // Should be A[aRow * N + aCol]

        Bs[threadIdx.y][threadIdx.x] =
            (bRow < N && bCol < K) ? B[bCol * N + bRow] : 0.0f;  // Should be B[bRow * K + bCol]
