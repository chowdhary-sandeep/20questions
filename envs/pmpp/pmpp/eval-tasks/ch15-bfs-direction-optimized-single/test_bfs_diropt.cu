// ch15-bfs-direction-optimized-single / test_bfs_diropt.cu
...
int main(){
    printf("ch15-bfs-direction-optimized-single tests\n");
    std::vector<CSR> gs;
    gs.push_back(make_chain(64));        // stays mostly push
    gs.push_back(make_star(1024));       // triggers pull (frontier ~V)
    gs.push_back(make_grid2d(16,16));    // may switch based on heuristic
    gs.push_back(make_erdos(400,0.03f)); // mixed
    ...
    for(size_t gi=0; gi<gs.size(); ++gi){
        ...
        bfs_direction_optimized_gpu(d_row, d_col, V, E, src, d_lvl);
