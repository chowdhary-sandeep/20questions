...
int main(){
    printf("ch09 shared-memory privatized histogram — tests\n");
      ...
      for(size_t N : sizes){
        for(int bs : blocksizes){
          for(int pat=0; pat<4; ++pat){
            ...
            // shared memory size for histogram
            size_t smem_size = nb * sizeof(unsigned int);

            // launch
            histogram_kernel<<<blocks, bs, smem_size>>>(d_in, d_hist, N, nb);
