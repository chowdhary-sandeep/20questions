// ch18-energy-gather-coarsened-coalesced-single / test_energy_gather_coarsened_coalesced.cu
...
extern "C" __global__
void cenergyCoarsenCoalescedKernel(float* energygrid, dim3 grid, float gridspacing, float z,
...
int main(){
    std::printf("ch18-energy-gather-coarsened-coalesced-single tests\n");
        ...
        for (int NA: atom_counts){
            ...
            auto run_mode = [&](bool shuffle){
                ...
                for (int idx=0; idx<chunks; ++idx){
                    ...
                    cenergyCoarsenCoalescedKernel<<<grid,block>>>(d_out, cfg.g, h, z, count, start);
